...
var postListObject = [
    ...
    },
    {
        ...
        content_body : "One of the biggest challenges in software development is time. It's something we can't easily get more of it but linting can help us make the most out of the time we have."
    },
    {
    ...
    {
        name: "Srishti Gupta",
        content_heading : "Everything you should know about module and require in Node.js",
        content_body : "Node.js treats each JavaScript file as a separate module. For instance if you have a file containing some code and this file is named xyz.js then this file is treated as a module in Node and you can say that you have created a module named xyz."
    }
];
...
function showProject(){
    ...
    postListObject.forEach(function(value,index){
        ...
        '<div class="input-group-prepend">' +
            '<p class="input-group-text name-design" id="'+ userCardId +'_name">' + value.name + '</p>' +
        '</div>' +

        '<div class="form-control content-design">' +
        '<div>' +
            '<p class="content-heading"><span id="'+ userCardId +'_content_heading">' + value.content_heading + '</span>' +
                '<span class="float-right cursor"><a onclick="deleteModal('+ userCardId +')"><i class="fa fa-trash" aria-hidden="true"></i></a></span>'+
            '</p>'+
        '</div>' +
        '<p class="content-body" id="'+ userCardId +'_content_body">' + value.content_body + '</p>' +
        '<span>' +
            '<span class="float-right cursor"><a onclick="gotopost(' + userCardId + ')"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></a></span>' +
        '</span></div></div></span>';
...
function gotopost(usercardId) {
    var name_id = usercardId.id + '_name';
    var content_heading_id = usercardId.id + '_content_heading';
    var content_body_id = usercardId.id + '_content_body';
    var name = document.getElementById(name_id).innerHTML;
    var content_heading = document.getElementById(content_heading_id).innerHTML;
    var content_body = document.getElementById(content_body_id).innerHTML;
    var url = '../html/post.html?name=' + encodeURIComponent(name) + '&content_heading=' + encodeURIComponent(content_heading) + '&content_body=' + encodeURIComponent(content_body);
